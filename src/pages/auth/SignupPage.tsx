...
export default function SignupPage() {
  ...
  const handleGoogle = async () => {
    setError("");
    try { await loginWithGoogle(); navigate("/setup-workspace"); }
    ...
    <div className="min-h-screen bg-background flex items-center justify-center p-4">
      <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} className="w-full max-w-md">
        ...
        <div className="glass rounded-2xl p-8 shadow-soft border border-border/50">
          ...
          <p className="text-muted-foreground text-sm mb-6">Start your growth journey</p>
          ...
          <button onClick={handleGoogle} disabled={authLoading}
            className="w-full flex items-center justify-center gap-3 py-2.5 rounded-lg border border-border bg-card hover:bg-muted transition-colors text-sm font-medium text-foreground mb-4 disabled:opacity-50">
          ...
          <form onSubmit={handleSubmit} className="space-y-4">
            {[
              { label: "Full Name", value: displayName, setter: setDisplayName, type: "text", placeholder: "Hajar Bell", Icon: User },
            ...
            {[
              ...
              { label: "Confirm Password", value: confirmPassword, setter: setConfirmPassword, placeholder: "••••••••" },
              ...
              <div key={label}>
                ...
                <div className="relative">
                  ...
                  <input type={showPassword ? "text" : "password"} value={value} onChange={(e) => setter(e.target.value)}
                    placeholder={placeholder} required
                    className="w-full pl-10 pr-10 py-2.5 rounded-lg bg-muted/50 border border-border text-sm text-foreground placeholder:text-muted-foreground focus:outline-none focus:ring-2 focus:ring-ring" />
                  {label === "Password" && (
