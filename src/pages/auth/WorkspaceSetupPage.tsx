...
export default function WorkspaceSetupPage() {
  const { createWorkspace, authLoading, user } = useAuth();
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try { await createWorkspace(name.trim(), type); navigate("/"); }
    catch { setError("Failed to create workspace. Please try again."); }
  };
    ...
    <div className="min-h-screen bg-background flex items-center justify-center p-4">
      <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} className="w-full max-w-md">
        ...
        <div className="flex items-center gap-2 justify-center mb-6">
          ...
          <span className="text-xs font-medium text-foreground">Setup workspace</span>
        </div>

        <div className="glass rounded-2xl p-8 shadow-soft border border-border/50">
          <h1 className="text-2xl font-bold font-display text-foreground mb-1">Hey {firstName}! 👋</h1>
          <p className="text-muted-foreground text-sm mb-6">Set up your workspace to get started.</p>
          ...
          <form onSubmit={handleSubmit} className="space-y-5">
            <div>
              ...
              <input type="text" value={name} onChange={(e) => setName(e.target.value)}
                placeholder="e.g. RuProof Agency" required
            ...
            <div>
              <label className="text-xs font-medium text-muted-foreground mb-2 block">Workspace Type</label>
              <div className="grid grid-cols-2 gap-3">
                {([["personal", User, "Personal", "Just for me"], ["agency", Building2, "Agency", "Team workspace"]] as const).map(([t, Icon, label, sub]) => (
                  <button key={t} type="button" onClick={() => setType(t)}
                    ...
                    <div className={`p-2.5 rounded-lg ${type === t ? "gradient-primary" : "bg-muted"}`}>
                      <Icon className={`w-5 h-5 ${type === t ? "text-primary-foreground" : "text-muted-foreground"}`} />
                    </div>
            ...
            <button type="submit" disabled={authLoading}
              ...
              {authLoading ? "Creating workspace..." : "Launch My Workspace 🚀"}
            </button>
          </form>
        </div>
      </motion.div>
